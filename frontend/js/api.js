...
async function handleLoginSubmit({ username, password, expectedRole, wrongRoleMessage, form, msgEl, submitBtn }) {
  ...
  try {
    ...
    // The response includes access_token for API/script clients, but the
    // frontend never persists it — only these display fields go to
    // localStorage. Auth itself rides on the httpOnly cookie already set
    // by this response.
    const { user_id, role, full_name, school_id } = data;
